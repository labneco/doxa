...
The models API allows modules to interact with Doxa's models and records. This
...
Special care has been put in Doxa's model API to ease development. In
particular:
...
When developing a Doxa module, you can create your own models and/or
...
[IMPORTANT]
====
After creating or modifying a model, you must run `doxa generate` to
generate the types in the `h` and `q` packages before starting the Doxa server.

Running `doxa generate` will also allow you to obtain code completion and
