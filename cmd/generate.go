...
func init() {
	DoxaCmd.AddCommand(generateCmd)
...
func runGenerate() {
	poolDir := filepath.Join(generate.DoxaDir, PoolDirRel)
	...
	fmt.Println(`Doxa Generate
--------------`)
	fmt.Printf("Detected Doxa root directory at %s.\n", generate.DoxaDir)
...
func createModuleSymlinks(mod *build.Package) {
	for _, dir := range symlinkDirs {
		...
		dstPath := filepath.Join(generate.DoxaDir, "doxa", "server", dir, mod.Name)
...
func cleanModuleSymlinks() {
	for _, dir := range symlinkDirs {
		dirPath := filepath.Join(generate.DoxaDir, "doxa", "server", dir)
