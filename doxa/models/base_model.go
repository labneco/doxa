...
func declareModelMixin() {
	idSeq := NewSequence("DoxaExternalID")

	modelMixin := NewMixinModel("ModelMixin")
	modelMixin.AddFields(map[string]FieldDefinition{
		"DoxaExternalID": CharField{Unique: true, Index: true, NoCopy: true, Required: true,
			Default: func(env Environment) interface{} {
				return fmt.Sprintf("__doxa_external_id__%d", idSeq.NextValue())
			},
		},
		"DoxaVersion": IntegerField{GoType: new(int)},
	})
...
func declareCRUDMethods() {
	...
	commonMixin.AddMethod("Copy",
		...
		func(rc *RecordCollection, overrides FieldMapper, fieldsToUnset ...FieldNamer) *RecordCollection {
			...
			rc.Load()
			newRs := rc.WithContext("doxa_force_compute_write", true).Call("Create", fMap).(RecordSet).Collection()
			return newRs
		})
