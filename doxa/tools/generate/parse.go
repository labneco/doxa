...
const (
	...
	Subs
	// Models is the PackageType for the doxa/models package
	Models
)
...
// GetModulePackages returns a slice of PackageInfo for packages that are doxa modules, that is:
// - A package that declares a "MODULE_NAME" constant
// - A package that is in a subdirectory of a package
// Also returns the 'doxa/models' package since all models are initialized there
func GetModulePackages(program *loader.Program) []*ModuleInfo {
	...
	// and we check for 'doxa/models' package
